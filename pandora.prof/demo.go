...
import (
	...
	"qiniu.com/pandora/sdd/executor"
)
...
type repoGroup struct {
	...
	Groups []qGroup `bson:"groups"` //根据start time排序的qGroup
}
...
type qGroup struct {
	...
	End      int64       `bson:"end"`
	InfluxDs []*qInfluxD `bson:"influxds"`
}

type qInfluxD struct {
	Id        string `bson:"id"`
	Host      string `bson:"host"`
	Port      string `bson:"port"`
	Reachable bool   `bson:"reachable"` //是否正常工作，如果PointD发现此处为false,表明该influxd挂了,由scheduler设置
	Offset    int64  `bson:"offset"`    //该influxd实例对应的consumer挂之前消费到的offset,由pointD设置
}

type GetbRepo struct {
	Id         string `bson:"id"`
	Name       string `bson:"name"`
	Region     string `bson:"region"`
	ShareLevel string `bson:"sharelevel"`
}

func WritePoints(args *cmdArgs, env *rpcutil.Env) (err error) {

	repoId := args.CmdArgs[1] + NAME_SPLITTER + args.CmdArgs[0]
		...
		Groups: []qGroup{
			{
				Id:       "1",
				Start:    12,
				End:      1555699465618258620,
				InfluxDs: []*qInfluxD{{Id: "localhost:8086", Host: "localhost", Port: "8086", Reachable: true}},
			},
		},
	}

	if len(getbGroup.Groups) > 2 {
		getbGroup.Groups = getbGroup.Groups[len(getbGroup.Groups)-2:]
	}

	points := make([]string, len(getbGroup.Groups))

	var req []byte
	if req, err = ioutil.ReadAll(env.Req.Body); err != nil {
		return
	}
	...
	for _, pt := range pts {
		seriesName := pt.Name()
		pt.SetName(repoId + NAME_SPLITTER + seriesName)

		if flag || lastSeries != seriesName {
			...
			// 写点的时候暂时不支持跨retention去写点，但是可以支持夸series写点
			if retention != rp {
				return
			}
		}

		for k, v := range getbGroup.Groups {
			if pt.Time().UnixNano() >= v.Start && pt.Time().UnixNano() <= v.End {
				points[k] = points[k] + pt.String() + "\n"
				break
			}

			if k == len(getbGroup.Groups)-1 {
				return
			}
		}
	}

	//sddExecutor := executor.NewSddExecutor()

	for k, g := range getbGroup.Groups {
		influxds := make([]*executor.Influxd, 0)
		for _, i := range g.InfluxDs {
			influxds = append(influxds, &executor.Influxd{
				Id:        i.Id,
				Host:      i.Host,
				Port:      i.Port,
				Start:     g.Start,
		...
		if points[k] != "" {
			/*if err = sddExecutor.WritePoints(influxds, points[k], rp, s.InfluxDBClient); err != nil {
				return
			}*/
		}
	}
